...
interface RepositoryUseCases<in REPOSITORY: Repository<T>, T: Any> {
    ...
    interface GetStatus<in REPOSITORY: Repository<T>, T: Any>:
            ()->StateFlow<SyncStatus<T>>

    interface GetLatestStatus<in REPOSITORY: Repository<T>, T: Any>:
            ()->StateFlow<T?>

    interface UpsertLatestItemIfDifferent<in REPOSITORY: Repository<T>, T: Any>:
        suspend (T)->RepositoryEntry<T>

    interface ClearEntries<in REPOSITORY: Repository<T>, T: Any>:
        suspend ()->Unit
}
