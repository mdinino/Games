...
class CoroutineCriticalSection(
    ...
    constructor(
        ...
        on = SupervisorScope(on.coroutineContext, parent),
        lock = lock
    )
    ...
    constructor(
        ...
        on = SupervisorScope(on.coroutineContext, parent.coroutineContext),
        lock = lock
    )
    ...
    constructor(
        ...
        on = SupervisorScope(on, parent.coroutineContext),
        lock = lock
    )

    constructor(
        on: CoroutineContext = Dispatchers.Default,
        parent: CoroutineCriticalSection,
        lock: Mutex = Mutex()
    ) : this(
        on = SupervisorScope(on, parent.on.coroutineContext),
        lock = lock
    )

    suspend fun <T> lockAndRun(block: suspend () -> T) =
        withContext(on.coroutineContext) {
            lock.withLock(this) {
                block()
            }
        }
    ...
    companion object {
        ...
        private fun SupervisorScope(
                on: CoroutineContext = Dispatchers.Default,
                parent: CoroutineContext) =
            CoroutineScope(on + parent + SupervisorJob())
    }
}